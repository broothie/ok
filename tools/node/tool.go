...
var definitionRegexp = regexp.MustCompile(`^function (?P<name>\w[a-zA-Z0-9_]*)\((?P<paramList>[^)]*)\) \{$`)
...
func (Tool) Init() error {
	return util.InitFile("Okfile.js", nil)
}

func (t Tool) ProcessFile(path string) ([]task.Task, error) {
	content, err := os.ReadFile(path)
	if err != nil {
		return nil, errors.Wrap(err, "failed to read javascript file")
	}
	...
	for i, line := range lines {
		...
		for _, param := range util.SplitCommaList(paramList) {
			...
			case 1:
				...
				params = append(params, task.PositionalParameter(paramName, task.TypeString))
			...
			case 2:
				...
				params = append(params, task.KeywordParameter(paramName, typ, paramDefault))
